export const senkouSkills = {
  inherit: [],
  inheritComments: [
    `Leaders with long-lasting speed mid-leg ults and strong midlegs can consider runner inherits`,
  ],
  recc: [
    {
      goldSkill: "怪物",
      goldEventCards: [30152],
      whiteEventCards: [""],
      comments: "",
    },
  ],
};

// {
//   id: 105101,
//   enName: "OG Nishino Flower",
//   comments: "3rd to 4th placement",
// },
// {
//   id: 101001,
//   enName: "Og Taiki Shuttle",
//   comments: "3rd to 4th placement",
// },
// {
//   id: 107801,
//   enName: "Yamanin Zephyr",
//   comments:
//     "Has a pretty weak effect to make up for the long duration. You'd rather want Nishino and Taiki inherits",
// },
// {
//   goldSkill: "プランX",
//   goldEventCards: [30020, 30133],
//   whiteEventCards: [""],
//   comments: "",
// },
// {
//   goldSkill: "鍔迫り合い",
//   goldEventCards: [30130, 30093],
//   whiteEventCards: [""],
//   comments: "",
// },
// {
//   goldSkill: "意気衝天",
//   goldEventCards: [30083],
//   whiteEventCards: [""],
//   comments:
//     "Pretty powerful skill to push you ahead of the pack with potential carry-over speed",
// },
// {
//   goldSkill: "決意の直滑降",
//   goldEventCards: [30031, 30127, 30151],
//   whiteEventCards: [""],
//   comments:
//     "It has...decent odds for gains. I wouldn't be on it though but i'll put it here.",
// },
// {
//   goldSkill: "ロケットスタート",
//   goldEventCards: [30184],
//   whiteEventCards: [""],
//   comments: "Insert Comment Here",
// },

//    {
//   goldSkill: "鍔迫り合い",
//   goldEventCards: [30130, 30093],
//   whiteEventCards: [""],
//   comments:
//     "Same effect as ハイボルテージ (HIGH VOLTAGE) so this is also good ",
// },
// {
//   goldSkill: "ハイボルテージ",
//   goldEventCards: [30153],
//   whiteEventCards: [""],
//   comments: "Best front-line accel",
// },
// {
//   goldSkill: "決意の直滑降",
//   goldEventCards: [30031, 30127, 30151],
//   whiteEventCards: [""],
//   comments:
//     "I'll put it here but the average gain is pretty awful compared to the above",
// },
